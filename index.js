...
const messageTemplates = {
    ...
    waiting: {
  message: `We're currently waiting on a specific staff member to handle your ticket. Thank you for your patience while we wait for them to get online!`,
  description: 'Notify user about escalation'
},
...
async function handleTicketModalSubmit(interaction) {
    ...
    // FIX: createTicketChannel'ın döndürdüğü channel'ı yakalıyoruz.
    // Öncesinde burada tanımsız bir "channel" değişkenine erişiliyordu ve
    // ReferenceError fırlatıp interaction.editReply hiç çalışmıyordu.
    const channel = await createTicketChannel(interaction.user, ticketType, answers);
...
saveData();
    ...
    // FIX: handleTicketModalSubmit'in kullanabilmesi için kanalı döndürüyoruz.
    return channel;
}
...
async function handleCloseCommand(message) {
    ...
    // FIX: sadece ManageMessages izni kontrol ediliyordu, admin/manage/staff
    // rolüne sahip ama bu tekil izne sahip olmayan kişiler için -close çalışmıyordu.
    const member = message.member ?? await message.guild.members.fetch(message.author.id).catch(() => null);
